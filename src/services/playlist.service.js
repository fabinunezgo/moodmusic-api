...
const PlaylistService = {
  ...
  getByUser: async (usuario_id) => {
    const [rows] = await pool.query("SELECT * FROM playlists WHERE usuario_id = ?", [usuario_id]);
    return rows;
  },
  ...
  update: async (id, { nombre, descripcion }) => {
    await pool.query("UPDATE playlists SET nombre = ?, descripcion = ? WHERE id = ?", [nombre, descripcion, id]);
